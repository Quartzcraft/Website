...
class QCGames_ControllerPublic_LabDefence extends XenForo_ControllerPublic_Abstract {

	public function actionIndex() {
		$options = XenForo_Application::get('options');
		
		if(!XenForo_Visitor::getInstance()->hasPermission('games', 'labdefence') && !XenForo_Visitor::getInstance()->isSuperAdmin() || !$options->displayLabDefence && !XenForo_Visitor::getInstance()->isSuperAdmin()) {
		...
		return $this->responseView('QCGames_ViewPublic_LabDefence_Guide', 'labdefence_index', $viewParams);
	}
	
	public function actionLeaderboards() {
		$options = XenForo_Application::get('options');

		if(!XenForo_Visitor::getInstance()->hasPermission('games', 'labdefence') && !XenForo_Visitor::getInstance()->isSuperAdmin() || !$options->displayLabDefence && !XenForo_Visitor::getInstance()->isSuperAdmin() || !$options->displayLabDefenceLeaderboard && !XenForo_Visitor::getInstance()->isSuperAdmin()) {
			return $this->responseNoPermission();
		}
		...
		return $this->responseView('QCGames_ViewPublic_LabDefence_Leaderboard', 'labdefence_leaderboard', $viewParams);
	}
	...
	public static function getSessionActivityDetailsForList(array $activities) {
		return new XenForo_Phrase('viewing_game_labdefence');
	} 
}
